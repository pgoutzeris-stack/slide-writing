...
logic to :mod:`deckforge.api` / :mod:`deckforge.spec`, and prints machine
...
from .errors import DeckForgeError, SpecError
...
def main(argv: list[str] | None = None) -> int:
    """Run the DeckForge CLI and return a process exit code."""
    parser = _parser()
    ...
    except (DeckForgeError, OSError, json.JSONDecodeError) as exc:
        print(f"deckforge: error: {exc}", file=sys.stderr)
        return 1
...
def _parser() -> argparse.ArgumentParser:
    ...
    bootstrap.add_argument("--name", default="DeckForge", help="Theme/template name")
